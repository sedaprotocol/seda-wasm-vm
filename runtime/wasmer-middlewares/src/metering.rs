...
impl<F: Fn(&Operator) -> u64 + Send + Sync + 'static> ModuleMiddleware for Metering<F> {
    ...
    fn transform_module_info(&self, module_info: &mut ModuleInfo) -> Result<(), MiddlewareError> {
        ...
        let remaining_points_global_index = module_info.globals.push(GlobalType::new(Type::I64, Mutability::Var));
        ...
        let points_exhausted_global_index = module_info.globals.push(GlobalType::new(Type::I32, Mutability::Var));
...
impl<F: Fn(&Operator) -> u64 + Send + Sync> FunctionMiddleware for FunctionMetering<F> {
    fn feed<'a>(
        ...
        // This needs to be done before the metering logic, to prevent operators like `Call` from escaping metering in
        // some corner cases.
...
mod tests {
    ...
    fn get_remaining_points_works() {
        ...
        let add_one: TypedFunction<i32, i32> = instance.exports.get_function("add_one").unwrap().typed(&store).unwrap();
        ...
        assert_eq!(get_remaining_points(&mut store, &instance), MeteringPoints::Exhausted);
    }
